/**
 * Standalone script to create Stripe products without env validation
 * This bypasses the chicken-and-egg problem with env.ts requiring price IDs that don't exist yet
 */
...
// Create Stripe client directly without env validation
...
export async function createStripeProducts() {
  ...
  try {
    ...
    const proYearlyPrice = await stripe.prices.create({
      product: proProduct.id,
      currency: "eur",
      unit_amount: 75900, // 759€ en centimes (économie de 189€)
      recurring: {
        interval: "year",
      },
      metadata: {
    ...
    const premiumMonthlyPrice = await stripe.prices.create({
      ...
      currency: "eur",
      unit_amount: 14900, // 149€ en centimes
      recurring: {
        interval: "month",
      },
      metadata: {
    ...
    const premiumYearlyPrice = await stripe.prices.create({
      product: premiumProduct.id,
      currency: "eur",
      unit_amount: 143000, // 1430€ en centimes (économie de 358€)
      recurring: {
        interval: "year",
      },
      metadata: {
